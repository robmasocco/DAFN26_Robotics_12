...
\begin{frame}{Docker Engine}
    ...
    \justifying
		...
		\bigskip
		It is an engine (\emph{i.e.}, a collection of \textbg{daemons}) that automates the management of the kernel subsystems in order to \textbg{set up}, \textbg{store}, and \textbg{run containers}.\\
    \bigskip
...
\begin{frame}{Building a Docker container}{Step by step}
  \justifying
	\begin{enumerate}
      ...
      \item \textbg{Images} are the binary archives from which a \textbg{container} can be started: they can be \textbg{stored}, \textbg{pulled} from a remote \textbg{registry} or simply \textbg{built} locally.
		\item A \textbg{container} can be built from an image and then \textbg{started}, \textbg{stopped}, and \textbg{managed} by the Docker daemon.
		\item \textbg{Processes} started \textbg{inside} the container are subject to its limitations, \emph{e.g.}, \textbg{filesystem jails} prevent them to climb up to the host filesystem.
	\end{enumerate}
...
\begin{frame}{Containers on real robots}{Best practices}
  ...
  \begin{itemize}
    ...
    \item your workspace directory should be \textbg{mounted as a volume} inside the container, so that file manipulations happen on the host filesystem (\texttt{-v /your/code:/home/user/workspace}).
  \end{itemize}
